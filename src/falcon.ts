    ...
    return {
        ...
        decode(data: TArg<Uint8Array>): TRet<Int16Array> {
            ...
            const readBits = (n: number) => {
                ...
                if (bufLen < n)
                    throw new Error(
                        `end of buffer: len=${bufLen} buf=${buf} lastByte=${data[pos]}`,
                    );
                bufLen -= n;
...
function getComplex<T>(field: IField<T>) {
    ...
    return {
        lift: (x: ComplexElm<T> | T): ComplexElm<T> => {
            ...
            if ((x as any).re !== undefined && (x as any).im !== undefined)
                return x as ComplexElm<T>;
        ...
        eql: (a: ComplexElm<T>, b: ComplexElm<T>): boolean =>
            F.eql(a.re, b.re) && F.eql(a.im, b.im),
    ...
    class NTRU {
        ...
        generate(): [SPoly, SPoly, SPoly, SPoly, IPoly] {
            ...
            while (true) {
                ...
                rt1 = floatPoly.iFFT(
                    floatPoly.scaleNorm(floatPoly.mulConst(floatPoly.conj(rt1), Q), rt3),
                );
                rt2 = floatPoly.iFFT(
                    floatPoly.scaleNorm(floatPoly.mulConst(floatPoly.conj(rt2), Q), rt3),
                );
