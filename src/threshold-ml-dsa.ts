...
import { MLDSAPrimitives, N, Q } from './ml-dsa-primitives.ts';
...
export class ThresholdMLDSA {
    static readonly #DKG_RHO_COMMIT = /* @__PURE__ */ new TextEncoder().encode('DKG-RHO-COMMIT');
    static readonly #DKG_BSEED_COMMIT = /* @__PURE__ */ new TextEncoder().encode(
        'DKG-BSEED-COMMIT',
    );
    static readonly #DKG_RHO_AGG = /* @__PURE__ */ new TextEncoder().encode('DKG-RHO-AGG');
    static readonly #DKG_GEN_ASSIGN = /* @__PURE__ */ new TextEncoder().encode('DKG-GEN-ASSIGN');
    static readonly #DKG_BSEED = /* @__PURE__ */ new TextEncoder().encode('DKG-BSEED');

    /** ML-DSA-44 threshold parameters: [K_iter, r, rPrime] indexed by [T-2][N-2]. */
    // prettier-ignore
    static readonly #PARAMS_44: Record<string, [number, number, number][]> = {
        '2': [[2, 252778, 252833]],
    ...
    // prettier-ignore
    static readonly #PARAMS_65: Record<string, [number, number, number][]> = {
        '2': [[2, 344000, 344080]],
    ...
    // prettier-ignore
    static readonly #PARAMS_87: Record<string, [number, number, number][]> = {
        '2': [[2, 442000, 442100]],
    ...
    static readonly #ALL_PARAMS: Record<number, Record<string, [number, number, number][]>> = {
    ...
    static readonly #POLY_Q_SIZE = (N * 23) / 8; // 736

    public static readonly MAX_PARTIES = 6;

    readonly #primitives: MLDSAPrimitives;
    public readonly params: ThresholdParams;
     ...
     */
    public static create(securityLevel: number, T: number, N_: number): ThresholdMLDSA {
    ...
    public static getParams(T: number, N_: number, securityLevel: number): ThresholdParams {
    ...
    static #getDSAOpts(securityLevel: number) {
        let paramKey: string;
    ...
    static #normalizeSecurityLevel(level: number): number {
    ...
    static #encodeU8(v: number): Uint8Array {
        return new Uint8Array([v & 0xff]);
    }

    static #encodeU16LE(v: number): Uint8Array {
        return new Uint8Array([v & 0xff, (v >> 8) & 0xff]);
    }

    /** Fill polynomial with uniform random values in [0, Q). */
    static #fillUniformModQ(poly: Int32Array): void {
        let filled = 0;
        while (filled < N) {
    ...
    static #getSharingPattern(T: number, N_: number): readonly (readonly number[])[] | null {
    ...
    static #polyPackW(p: Int32Array, buf: Uint8Array, offset: number): void {
        let v = 0;
        let j = 0;
        let k = 0;
        for (let i = 0; i < N; i++) {
    ...
    static #polyUnpackW(p: Int32Array, buf: Uint8Array, offset: number): void {
        let v = 0;
        let j = 0;
        let k = 0;
        for (let i = 0; i < N; i++) {
    ...
    static #packPolys(polys: Int32Array[][], dim: number, K_iter: number): Uint8Array {
    ...
    static #unpackPolys(buf: Uint8Array, dim: number, K_iter: number): Int32Array[][] {
    ...
    /** Add two FVecs. */
    static #fvecAdd(a: Float64Array, b: Float64Array): Float64Array {
    ...
    static #fvecExcess(v: Float64Array, r: number, nu: number, K: number, L: number): boolean {
        let sq = 0;
        for (let i = 0; i < L + K; i++) {
            for (let j = 0; j < N; j++) {
                const val = v[i * N + j];
    ...
    static #fvecRound(v: Float64Array, K: number, L: number): { z: Int32Array[]; e: Int32Array[] } {
        const z: Int32Array[] = [];
        const e: Int32Array[] = [];
     ...
     */
    public keygen(seed?: Uint8Array): ThresholdKeygenResult {
     ...
     */
    public dkgSetup(sessionId: SessionId): DKGSetupResult {
        ...
        for (const b of bitmasks) {
            ...
            for (const j of holders) {
                ...
                if (!msg) {
                    msg = {
                        fromPartyId: partyId,
                        bitmaskReveals: new Map<number, Uint8Array>(),
                    };
            ...
            for (let j = 0; j < params.N; j++) {
                ...
                if (!msg) {
                    msg = {
                        ...
                        maskPieces: new Map<number, Int32Array[]>(),
                    };
    ...
    #aggregateCommitments(allWs: Int32Array[][][], params: ThresholdParams): Int32Array[][] {
        const p = this.#primitives;
        const { K } = p;
    ...
    #aggregateResponses(allZs: Int32Array[][][], params: ThresholdParams): Int32Array[][] {
        const p = this.#primitives;
        const { L } = p;
        ...
        for (let iter = 0; iter < params.K_iter; iter++) {
            ...
            for (let i = 0; i < K; i++) {
                const ct12d = p.MultiplyNTTs(
                    p.NTT.encode(p.polyShiftl(t1[i].slice() as Int32Array)),
                    cHat,
                );
