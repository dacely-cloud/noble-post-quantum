            ...
            for (const g of loadACVP('ML-KEM-keyGen-FIPS203')) {
                ...
                for (const t of g.tests) {
                    const { publicKey, secretKey } = mlkem.keygen(
                        concatBytes(hexx(t.p.d), hexx(t.p.z)),
                    );
                ...
                for (const t of g.tests) {
                    ...
                    if (g.info.p.signatureInterface === 'internal') {
                        if (g.info.p.externalMu)
                            sig = mldsa.internal.sign(hexx(t.p.mu), hexx(t.p.sk), opts);
                        ...
                        if (g.info.p.preHash === 'preHash') {
                            ...
                            sig = mldsa
                                .prehash(hash)
                                .sign(hexx(t.p.message), hexx(t.p.sk), optsCtx);
                        } else {
                ...
                for (const t of g.tests) {
                    ...
                    if (g.info.p.signatureInterface === 'internal') {
                        ...
                        if (g.info.p.preHash === 'preHash') {
                            ...
                            valid = mldsa
                                .prehash(hash)
                                .verify(hexx(t.p.signature), hexx(t.p.message), hexx(t.p.pk), {
                                    context: ctx,
                                });
                        } else {
                            valid = mldsa.verify(
            ...
            for (const g of all) {
                ...
                for (const t of g.tests) {
                    ...
                    should(`vector ${i} of ${total}`, () => {
                        const rnd = t.p.additionalRandomness
                            ? hexx(t.p.additionalRandomness)
                            : false;
                        let sig;
                        if (g.info.p.signatureInterface === 'internal') {
                            sig = slhdsa.internal.sign(hexx(t.p.message), hexx(t.p.sk), {
                                extraEntropy: rnd,
                            });
                        } else if (g.info.p.signatureInterface === 'external') {
                            const hash = HASHES[t.p.hashAlg];
                            ...
                            if (g.info.p.preHash === 'preHash') {
                                ...
                                sig = slhdsa
                                    .prehash(hash)
                                    .sign(hexx(t.p.message), hexx(t.p.sk), opts);
                            } else {
